# From Gurobi Tutorial P156
# Benders Decomposition的基本思想是通过将求解难度较大的原问题分解为两个更容易求解的问题master problem和subproblem
# 类似于列生成（Column Generation）方法，不同的是列生成方法通过求解subproblem产生column添加至master problem
# 而Benders Decomposition通过求解subproblem产生cut添加至主问题（类似row/ constraint generation）
# Tutorial: https://zhuanlan.zhihu.com/p/324556293
...
def addBendersCuts(SP_Dual_obj, x):
    if SP_Dual.status == GRB.Status.UNBOUNDED:  # 模型无界
        ray = SP_Dual.UnbdRay
        MP.addConstr(
            8 * ray[0] + 3 * ray[1] + 5 * ray[2] + 8 * ray[3] * y[0] + 3 * ray[4] * y[1] + 5 * ray[5] * y[2] + 5 * ray[
                6] * y[3] + 3 * ray[7] * y[4] <= 0)
    elif SP_Dual.status == GRB.Status.OPTIMAL:  # 发现最优解
        MP.addConstr(8 * Vdual1[0].x + 3 * Vdual1[1].x + 5 * Vdual1[2].x + 8 * Vdual2[0].x * y[0] + \
                     3 * Vdual2[1].x * y[1] + 5 * Vdual2[2].x * y[2] + 5 * Vdual2[3].x * y[3] + 3 * Vdual2[4].x * y[
                         4] <= z)
...
try:
    ...
    SP_Dual.Params.InfUnbdInfo = 1  # 设置参数InfUnbdInfo
    iteration = 0
    SP_Dual_obj = [9999]
    x = []
    MP.optimize()
    ...
    while z.x < SP_Dual_obj[0]:  # 迭代主循环
        if iteration == 0:
            SP_Dual.setObjective(8 * Vdual1[0] + 3 * Vdual1[1] + 5 * Vdual1[2] + \
                                 8 * Vdual2[0] * y[0].x + 3 * Vdual2[1] * y[1].x + 5 * Vdual2[2] * y[2].x + 5 * Vdual2[
                                     3] *
                                 y[3].x + 3 * Vdual2[4] * y[4].x, GRB.MAXIMIZE)
            SP_Dual.optimize()
    ...
    print('\n')
    for i in range(5):  # 获取变量x的取值
        print('x[%d] = %f' % (i, x[i]))
    for i in range(5):  # 获取变量y的取值
        print('y[%d] = %d' % (i, y[i].x))
